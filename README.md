...
The experiments show that increasing the L1 penalty on the learnable gates makes the network much sparser while keeping the accuracy drop relatively small. The best practical balance was achieved around $\lambda = 5.0$, where the model reached 97.0% sparsity with only a 1.1% absolute accuracy drop compared to the baseline.

Hard pruning produced the same accuracy as the soft-gated model, which confirms that the removed connections were already inactive and not contributing useful information during inference.

| Lambda ($\lambda$) | Test Accuracy | After Hard Prune | Sparsity | Drop vs Baseline |
| :--- | :--- | :--- | :--- | :--- |
| **0 (Baseline)** | 60.8% | 60.8% | 11.4% | 0.0% |
